...
# P1 items: items whose all inputs are not produced by any PI process (i.e., not in PI_DATA)
P1_ITEMS = [
    k for k, v in PI_DATA.items()
    if all(input_name not in PI_DATA for input_name in v.get("inputs", {}))
]

# P2, P3, P4 item determination logic (if needed)...
...
class CalculationScreen(tk.Frame):
    ...
    def load_tier(self, tier):
        ...
        # Only show items for the selected tier (e.g., P1)
